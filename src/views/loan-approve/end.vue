<template>
  <div class="app-container">
    ...
    <!-- 通过弹层功能 -->
    <el-dialog title="贷款审批-终审" :visible.sync="dialogPassVisible">
      <p>是否终审通过？</p>
    ...
    <!-- 拒绝弹层功能 -->
    <el-dialog title="贷款审批-终审" :visible.sync="dialogRefuseVisible">
      <p>是否终审拒绝？</p>
      <div slot="footer" class="dialog-footer">
...
export default {
  ...
  methods: {
    ...
    passData() {
      ...
      approveEndPass({ appId: tempData.id, loanId: tempData.loan_id }).then(
        (res) => {
          // console.log(res);
          if (res.data.code == "20000") {
    ...
    RefuseData() {
      const tempData = Object.assign({}, this.temp);
      approveEndReject({ appId: tempData.id, loanId: tempData.loan_id }).then(
        (res) => {
          if (res.data.code == "20000") {
            this.getList(); //调用查询接口
